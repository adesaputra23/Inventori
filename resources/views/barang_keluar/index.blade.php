                            ...
                            <tbody>
                                @foreach ($listBarangKeluar as $key => $barangKeluar)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td style="width: 15%">
                                            {{ \Carbon\Carbon::parse($barangKeluar->tanggal_keluar)->format('d M Y') }}</td>
                                        <td>{{ $barangKeluar->no_surat_keluar }}</td>
                                        <td>{{ Persediaan::gnerateKode($barangKeluar->id_persediaan) }}</td>
                                        <td class="@if ($barangKeluar->persediaan == null) text-center @endif">
                                            {{ $barangKeluar->persediaan->nama_barang ?? '-' }}</td>
                                        <td class="@if ($barangKeluar->ket_keluar == null) text-center @endif">
                                            {{ $barangKeluar->ket_keluar ?? '-' }}</td>
                                        @if (!User::isKepala())
                                            <td class="text-center">
                                                <button id="btn-edit" data-kode="{{ $barangKeluar->kode_barang_keluar }}"
                                                    type="button" class="btn-edit btn btn-warning btn-sm">Edit</button>
                                                <button type="button" data-kode="{{ $barangKeluar->kode_barang_keluar }}"
